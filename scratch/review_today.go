...
func main() {
	...
	for r1.Next() {
		...
		fmt.Printf("  %-24s %d trades  %dW  P&L=₹%.2f  AvgW=%.0f  AvgL=%.0f\n", s, c, w, t, aw, al)
	}
	r1.Close()
	...
	for r2.Next() {
		...
		icon := "✅"; if pnl < 0 { icon = "❌" }
		fmt.Printf("  %s %s %-20s %-12s E=%.1f X=%.1f Q=%d P&L=%s%.0f %s [%s RV=%.1f]\n",
			icon, t, s, sym, ep, xp, q, func()string{if pnl>=0{return "+"}; return ""}(), pnl, er, reg, rv)
	}
	r2.Close()
	...
	// Daily summary history (last 5)
	fmt.Println("\n═══ RECENT DAILY P&L ═══")
	r5, _ := db.Query(`SELECT date, total_trades, wins, losses, win_rate, gross_pnl FROM daily_summary ORDER BY date DESC LIMIT 5`)
	for r5.Next() {
		var d string; var tt, w, l int; var wr2, gp float64
		r5.Scan(&d, &tt, &w, &l, &wr2, &gp)
		icon := "🟢"; if gp < 0 { icon = "🔴" }
		fmt.Printf("  %s %s  %d trades  %dW/%dL  WR=%.0f%%  P&L=₹%.0f\n", icon, d, tt, w, l, wr2, gp)
	}
	r5.Close()
}
